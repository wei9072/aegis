...
def run_probe(probe_id: str, label: str, prompt: str, model: str) -> None:
    print("=" * 78)
    print(f"### {probe_id}-{label}  [model={model}]")
    print(f"prompt: {prompt[:100]}{'...' if len(prompt) > 100 else ''}")

    gateway = LLMGateway(llm_provider=GeminiProvider(model_name=model))
...
def main() -> int:
    ...
    for probe_id, (label, prompt) in selected:
        try:
            run_probe(probe_id, label, prompt, args.model)
        except Exception as e:
