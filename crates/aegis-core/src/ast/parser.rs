...
/// Pure-Rust import extraction. Returns the sorted, deduplicated
/// list of import names visible in the file.
...
mod tests {
    ...
    fn test_get_imports_returns_sorted_list() {
        ...
        tmp.flush().unwrap();
        let result = get_imports_native(tmp.path().to_str().unwrap()).unwrap();
